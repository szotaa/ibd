...
$stronicowanie = new Stronicowanie($_GET, $select["params"]);
$linki = $stronicowanie->pobierzLinki($select["sql"], 'admin.autorzy.lista.php');
$selectZPaginacja = $stronicowanie->dodajLimit($select["sql"]);
$lista = $autorzy->pobierzWszystko($selectZPaginacja, $select["params"]);
    ...
    <table id="autorzy" class="table table-striped">
        ...
        <?php foreach ($lista as $a): ?>
            <tr>
                ...
                <td><?= $a['imie'] ?></td>
                <td><?= $a['nazwisko'] ?></td>
                <td><?= $a['liczba_ksiazek'] ?></td>
                <td>
                    <a href="admin.autorzy.edycja.php?id=<?= $a['id'] ?>" title="edycja" class="aEdytujAutora"><em
                                class="fas fa-pencil-alt"></em></a>
                    <a href="admin.autorzy.usun.php?id=<?= $a['id'] ?>" title="usuń" class="aUsunAutora"><em
